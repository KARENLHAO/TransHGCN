...
def _estimate_density_from_files(label_file, Gene_file, TF_file):
    """当 Network_Statistic 返回 None/非法时，自行估计网络密度 d = |E+| / (|TF|*|Gene|)"""
    lab = (
        pd.read_csv(label_file, index_col=0)[["TF", "Target"]]
        .dropna()
        .drop_duplicates()
    )
    n_pos = len(lab)
    n_tf = pd.read_csv(TF_file, index_col=0)["index"].dropna().nunique()
    n_ge = pd.read_csv(Gene_file, index_col=0)["index"].dropna().nunique()
    denom = n_tf * n_ge
    if denom <= 0:
        # 兜底一个极小但合理的密度，避免除零
        return 1e-6
    d = n_pos / denom
    # 避免 0 或 1，保证分母有意义
    d = float(min(max(d, 1e-6), 1.0 - 1e-6))
    return d
...
def train_val_test_set(label_file,Gene_file,TF_file,train_set_file,val_set_file,test_set_file,density,p_val=args.p_val):
    ...
    for k in val_pos.keys():
        ...
        for i in range(len(val_pos[k])):
            ...
            while (
                (neg == k)
                or (neg in pos_dict.get(k, []))
                or (neg in train_neg.get(k, []))
                or (neg in val_neg.get(k, []))
            ):
                neg = np.random.choice(gene_set)
    ...
    test_neg_num = int(round(count * (1.0 - density) / density))
    test_neg_num = max(test_neg_num, 0)
    test_neg = {}
    for k in tf_set:
        test_neg[k] = []
...
if __name__ == '__main__':
    ...
    try:
        bad = (density is None) or (not np.isfinite(density)) or (density <= 0.0) or (density >= 1.0)
    except Exception:
        bad = True
    ...
    if bad:
        density = _estimate_density_from_files(label_file, Gene2file, TF2file)
        print(f"[Info] Network_Statistic 无效，改用回退密度 density={density:.6g}")
    else:
        print(f"[Info] 使用 Network_Statistic 返回的 density={density:.6g}")
